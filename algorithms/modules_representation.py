...
class DecoderGeneric(nn.Module):
    ...
    def forward(self, slots):
        ...
        # >>> TODO Note: encoder -> encoder's pos embedding, decoder's pos embedding -> decoder
...
class DecoderMedium(nn.Module):
    """
    TODO adopted from CSWM Decoder Medium version
    """
    ...
    def __init__(self,
        ...
        if use_embed:
            self.embed = nn.Sequential(
                ...
                nn.Linear(hidden_dims[0], hidden_dims[1]),
                # nn.Linear(hidden_dims[0], hidden_dims[-1]),  # TODO
                nn.LayerNorm(hidden_dims[1]),  # >>> based on the DecoderCNNSmall
                # nn.LayerNorm(hidden_dims[-1]),  # >>> based on the DecoderCNNSmall
                get_act_fn(act_fn),
