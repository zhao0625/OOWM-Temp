...
from algorithms.deprecated_modules_representation import EncoderHomomorphicSlotAttention, EncoderSelfAttention, EncoderHomomorphicAttention, EncoderHomomorphicSoftBinding
from algorithms.modules_representation import EncoderMLP
from algorithms.world_model import TransitionGNN, EncoderCNNSmall, EncoderCNNMedium, EncoderCNNLarge, TransitionMLP
...
class VanillaContrastiveSWM(nn.Module):
    ...
    # def contrastive_loss(self, obs, action, next_obs, extra_return=False):
    def compute_transition_loss(self, data_batch, with_neg_obs, with_vis=None, with_vis_recon=None):
        ...
        # > Encode to one-hot here!
        # TODO consider N objects
    ...
    def action2oh(self, action, reshape=True):
        ...
        if reshape:
            action_vec = action_vec.view(len(action), (self.num_objects_total if self.extra_filter else self.num_objects), self.action_dim)
    ...
    def predict(self, observations, actions, num_steps, space=None):
        ...
        with torch.no_grad():
            ...
            # >>> FIXME len(obs) = len(actions) + 1
            pred_state = state
            for i in range(num_steps):
                # TODO encode to latent space for every
